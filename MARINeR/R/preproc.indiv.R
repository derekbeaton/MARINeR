...
#'  @author Jenny Rieck
#'
preproc.indiv<-function(x, col.center = T, col.scale = F, row.center = F, row.scale = F, svd.rank.rebuild = F, rank.rebuild = NA, detrend.per.run=T, detrend.degrees=1, svd.norm = T){
  ...
  for(s in 1:length(subjs)){
    ...
    x[[subjs[s]]]$dataMatrix <- row.norms(x[[subjs[s]]]$dataMatrix,center=row.center,scale=row.scale)
